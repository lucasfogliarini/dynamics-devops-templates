function RenameFormsToManaged {
    ...
    if(Test-Path -Path $formsPath){
        $forms = Get-ChildItem $formsPath -Recurse -Include "*.xml" | Where-Object { ($_.FullName -match "FormXml") -and ($_.Name -notmatch "_managed") }
        ...
        Write-Host "Nenhum formulario foi encontrado."
    }
}
